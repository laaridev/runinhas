...
func (gi *GSIInstaller) Install() InstallResult {
	...
	if err != nil {
		...
		return InstallResult{
			Success: false,
			Message: "Não foi possível encontrar o Dota 2. Certifique-se de que está instalado via Steam.",
		}
	}
	...
	if err := os.MkdirAll(gsiDir, 0755); err != nil {
		...
		return InstallResult{
			Success: false,
			Message: fmt.Sprintf("Erro ao criar diretório: %v", err),
		}
	}
	...
	if _, err := os.Stat(configPath); err == nil {
		...
		return InstallResult{
			Success: true,
			Message: "Arquivo GSI já está instalado!",
	...
	if err := gi.writeConfigFile(configPath); err != nil {
		...
		return InstallResult{
			Success: false,
			Message: fmt.Sprintf("Erro ao escrever arquivo: %v", err),
		}
	}
	...
	return InstallResult{
		Success: true,
		Message: "GSI instalado com sucesso!",
		InstalledAt: configPath,
	}
}
